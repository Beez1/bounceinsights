...
const EmailDialog = ({ isOpen, onClose, onSend, email, setEmail }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-[#1c1c1c] rounded-lg p-8 shadow-xl max-w-sm w-full">
        <h2 className="text-xl font-bold text-white mb-4">Send to Email</h2>
        <p className="text-white/70 mb-6">Enter your email address to receive this content.</p>
        <form onSubmit={handleSend}>
          ...
          <div className="flex justify-end space-x-4 mt-6">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 rounded-md text-white/80 hover:bg-white/10 transition-colors"
            >
              Cancel
            </button>
            <button
